...
import { FiPackage, FiTruck, FiCheckCircle, FiClock, FiAlertCircle, FiDollarSign, FiActivity } from 'react-icons/fi';

const RiderDashboard = () => {
  const axiosSecure = useAxiosSecure();
  const riderEmail = "rider@example.com"; // Get from auth context

  // Fetch status counts for all parcels
  ...
  const { data: riderData } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(`/parcels/rider/${riderEmail}`);
      return res.data;
    },
    enabled: !!riderEmail
  });
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-2">Delivery performance overview</p>
        </div>
        <div className="mt-4 md:mt-0">
          <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
            <FiPackage className="mr-1" /> Total Assigned: {riderData?.totalParcels || 0}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        {/* Bar Chart */}
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">Parcel Status Overview</h2>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={statusCounts}
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {riderData?.recentParcels?.map(parcel => (
                <tr key={parcel._id}>
